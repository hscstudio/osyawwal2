...
use backend\modules\pusdiklat2\competency\models\planning\MeetingActivitySearch;;
use backend\modules\pusdiklat2\competency\models\planning\RoomSearch;
...
class MeetingActivity3Controller extends Controller
{
    ...
    public function actionPic($id)
    {
		...
		$renders['model'] = $model;
		$object_people_array = [
			// CEK ID 394 IN TABLE ORGANISATION IS SUBBIDANG PROGRAM
			'organisation_394'=>'PIC Meeting'
		];
    ...
    public function actionExportMeeting($year='',$status='all',$filetype='xlsx')
    {
		...
		$template = Yii::getAlias('@file').DIRECTORY_SEPARATOR.'template'.DIRECTORY_SEPARATOR.'pusdiklat'.
			DIRECTORY_SEPARATOR.'planning'.DIRECTORY_SEPARATOR.'meeting.list.'.$filetype;
