...
class ActivityGenerateController extends Controller
{
	...
	public function actionGenerateFormb($id,$filetype='docx')
    {
		...
		try {
			...
			{$template = Yii::getAlias('@file').'/template/bdk/evaluation/template_formb.docx';}
			else
			{$template = Yii::getAlias('@file').'/template/bdk/evaluation/template_formb2.docx';}
			...
			$data[] = [
						...
						'nama_satker_pusdiklat'=> $data_training->program->satker->name,
						...
						'jml_tdk_lulus'=>TrainingClassStudent::find()->where(['training_id'=>$id,'status'=>0])->count()." Orang",
						'jml_ulang'=>TrainingClassStudent::find()->where(['training_id'=>$id,'status'=>2])->count()." Orang",
