...
<div class="person-form">
	...
	<div class="tab-content" style="border: 1px solid #ddd; border-top-color: transparent; padding:10px; background-color: #fff;">
		...
		<?php 
		if(!$trainer->isNewRecord){ ?>
		<div class="tab-pane fade" id="trainer_information">
			<h3>Informasi Pengajar</h3>
			<?= $form->field($trainer, 'category')->textInput(['maxlength' => 255]) ?>
			<?= $form->field($trainer, 'education_history')->textarea(['rows' => 6]) ?>
			<?= $form->field($trainer, 'training_history')->textarea(['rows' => 6]) ?>
			<?= $form->field($trainer, 'experience_history')->textarea(['rows' => 6]) ?>
		...
		<div class="tab-pane fade" id="contact_information">
			...
			<a class="btn btn-default" onclick="$('#tab_wizard a[href=#<?= (!$trainer->isNewRecord)? 'trainer_information' : 'personal_information'; ?>]').tab('show')">
				Previous 
				<i class="fa fa-fw fa-arrow-circle-o-left"></i>
			</a>
