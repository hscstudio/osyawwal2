				...
				'value' => function ($data){
					...
					$object_person=\backend\models\ObjectPerson::find()
						->where([
							'object'=>'activity',
							'object_id'=>$data->id,														
							'type'=>'organisation_1213040100' //1213040100 CEK KD_UNIT_ORG 1213040100 IN TABLE ORGANISATION IS SUBBIDANG PENYEL I
						])
						->one();
					if($permit){
						$options = [
					...
					'dashboard' => function ($url, $model) {
								...
								if (in_array($model->status,[2])){
									return Html::a($icon,$url,[
										...
										'data-pjax'=>'0',
										'modal-size' => 'modal-lg',
										'modal-title' => '<i class="fa fa-fw fa-dashboard"></i> Dashboard '.$model->name
									]);
								}
							},
					...
					'cost-real' => function ($url, $model) {
								...
								if (in_array($model->status,[2])){
									return Html::a($icon,$url,[
										...
										'data-container' => 'body',
										'data-pjax'=>'0',
										'modal-title' => '<i class="fa fa-fw fa-money"></i> Realisasi Anggaran'
									]);
								}
							},
					...
					'cetak' => function ($url, $model) {
								...
								if (in_array($model->status,[2])) {
									...
									$tembolok .= Html::a($icon.' <span class="caret"></span>', null,[
										...
										'class'=>'btn btn-default btn-xs dropdown-toggle',
										'title' => 'Cetak Nilai Kehadiran, Aktivitas, Pre-Test, dan Post-Test',
										'data-toggle' => 'tooltip',
										'data-placement' => 'top',
										'data-container' => 'body',
										'data-pjax'=>'0',
									...
									foreach ($model->training->trainingClasses as $trainingClass) {
										$tembolok .= '<li>';
										$tembolok .= Html::a('Kelas '.$trainingClass->class, Url::to(['activity/cetak', 
											'training_id' => $trainingClass->training_id, 
											'training_class_id' => $trainingClass->id
										]));
										$tembolok .= '</li>';
